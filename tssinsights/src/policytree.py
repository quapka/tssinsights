...
class PolicyTree(PolicyNode):
    def __init__(self, threshold: int, nodes: tuple[PolicyNode]):
...
def generate_policy_trees(depth: int, max_children: int, roles: tuple[str], compile_callback, store_callback) -> set[PolicyNode]:
    """
...
#        unique_trees.update(generate_subtrees(sub_trees, num_children, compile_callback, store_callback))
        for child_combo in combinations(sub_trees, num_children):
            ...
            for threshold in range(1, num_children + 1):
                if threshold == 1 and num_children == 1:
                    tree = child_combo[0]
                else:
                    tree = PolicyTree(threshold, child_combo)
...
def generate_subtrees(sub_trees: tuple[PolicyNode], num_children, compile_callback, store_callback) -> tuple[PolicyNode]:
    unique_trees = set()
    for child_combo in combinations(sub_trees, num_children):
    # For each possible threshold (from 1 up to num_children)
        for threshold in range(1, num_children + 1):
